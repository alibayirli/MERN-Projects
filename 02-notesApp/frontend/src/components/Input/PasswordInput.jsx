...
const PasswordInput = ({value, onChange, placeholder}) => {
  ...
  return (
    <div className='flex items-center bg-transparent border-2 px-5 rounded mb-3'>
        ...
        {isShowPassword ? (<FaRegEye size={22} className="text-primary cursor-pointer" onClick={() => toggleShowPassword()}></FaRegEye>) 
        : (<FaRegEyeSlash size={22} className='text-slate-400 cursor-pointer' onClick={() => toggleShowPassword()}></FaRegEyeSlash>)
        }
    </div>
  )
}
